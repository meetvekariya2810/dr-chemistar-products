...
import { Image as ImageIcon, Video, Building2, FlaskConical, Users, Award, Play } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredItems.map((item) => (
            <div key={item.id} className="group relative rounded-3xl overflow-hidden border border-white/10 bg-slate-800 shadow-2xl">
              <img 
                src={item.image} 
                alt={item.title} 
                className="w-full h-64 object-cover group-hover:scale-105 transition-transform duration-500"
              />
              <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/40 to-transparent opacity-90 p-6 flex flex-col justify-end">
